...
class Liquidity(binomialTree):
    ...
    def illquidPrice(a0,k,ttm,rf,sharpR,sigma):
        ...
        EXTVT = math.pow(a0,2)*math.exp((2*mu+sigma*sigma)*ttm)*norm.cdf(dpp) - k*a0*math.exp(mu*ttm)*norm.cdf(dp)
    ...
    def BSPricer(s,k,rf,div,vol,T,pcFlag):
        ...
        return s*math.exp(-div*T)*pcFlag*n1 - k * math.exp(-rf*T)*pcFlag*n2
    
    def calibrateWealth(adjustmentFactor,rf,div,vol,T,pcFlag,equityTarket):
        return root(lambda w: Liquidity.BSPricer(w,w*adjustmentFactor,rf,div,vol,T,pcFlag)-equityTarket,[equityTarket]).x[0]
